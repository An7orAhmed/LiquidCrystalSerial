...
LiquidCrystalSerial::LiquidCrystalSerial(uint8_t rs, uint8_t rw, uint8_t enable,
...
LiquidCrystalSerial::LiquidCrystalSerial(uint8_t rs, uint8_t enable,
    uint8_t d0, uint8_t d1, uint8_t d2, uint8_t d3,
    uint8_t d4, uint8_t d5, uint8_t d6, uint8_t d7)
{
...
void LiquidCrystalSerial::init(uint8_t fourbitmode, uint8_t rs, uint8_t rw, uint8_t enable,
                               uint8_t d0, uint8_t d1, uint8_t d2, uint8_t d3,
                               uint8_t d4, uint8_t d5, uint8_t d6, uint8_t d7)
{
  _rs_pin = rs;
...
void LiquidCrystalSerial::begin(uint8_t cols, uint8_t lines, uint8_t mode, uint8_t dotsize) {
  if (lines > 1) {
  ...
  if (_operating_mode) {
    ...
    digitalWrite(_rs_pin, 0);
    digitalWrite(_enable_pin, 0);
    if (_rw_pin != 255) {
      digitalWrite(_rw_pin, 0);
    }
  }
  ...
  if (! (_displayfunction & LCD_8BITMODE)) {
    ...
    if (!_operating_mode) write4bits(0x03);
    else write4bits_595(0x03);
    ...
    if (!_operating_mode) write4bits(0x02);
    else write4bits_595(0x02);
  } else {
    // this is according to the hitachi HD44780 datasheet
    // page 45 figure 23
...
void LiquidCrystalSerial::setRowOffsets(int row0, int row1, int row2, int row3)
{
  _row_offsets[0] = row0;
  _row_offsets[1] = row1;
  _row_offsets[2] = row2;
...
inline void LiquidCrystalSerial::command(uint8_t value) {
  if(!_operating_mode) send(value, 0);
  else send_595(value, 0);
}

inline size_t LiquidCrystalSerial::write(uint8_t value) {
  if(!_operating_mode) send(value, 1);
  else send_595(value, 1);
...
void LiquidCrystalSerial::send(uint8_t value, uint8_t mode) {
...
void LiquidCrystalSerial::pulseEnable(void) {
  digitalWrite(_enable_pin, 0);
  delayMicroseconds(1);
...
void LiquidCrystalSerial::write4bits(uint8_t value) {
  for (int i = 0; i < 4; i++) {
...
void LiquidCrystalSerial::write8bits(uint8_t value) {
  for (int i = 0; i < 8; i++) {
    digitalWrite(_data_pins[i], (value >> i) & 0x01);
  }
